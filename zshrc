...
# Tmux ip for ctf 
# Set the attacker IP address
setattacker() {
    mkdir -p ~/.config/tmux
    echo "$1" > ~/.config/tmux/attacker_ip
}

# Set the target IP address
settarget() {
    mkdir -p ~/.config/tmux
    echo "$1" > ~/.config/tmux/target_ip
}

# Clear both IP addresses with a single command
donectf() {
    echo "No IP" > ~/.config/tmux/attacker_ip
    echo "No IP" > ~/.config/tmux/target_ip
 }

# Shortcut to read files quickly with ccze colorization
alias rcat='ccze -A <'
